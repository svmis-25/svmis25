...
if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id'])){
  ...
  $contact = $_POST['contact'];
    ...
    if (empty($errors)) {
		...
		$sql = "UPDATE users SET firstname = ?, middlename = ?, lastname = ?, qualifier = ?, contact = ?, email = ?, company_id = ?, role_id = ? WHERE id = ?";
		...
		if($stmt){
			//Bind parameters
			$stmt->bind_param("ssssssiii", $firstname, $middlename, $lastname, $qualifier, $contact, $email, $company, $role, $id);
