...
if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id'])){
    ...
    // Update the van data
    if (empty($errors)) {
      $sql = "UPDATE vans SET `status` = 'deactivated' WHERE id = ?";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("i", $id);
      if ($stmt->execute()) {
          //Record Activity
          require_once "activityLogsFunction.php";
          $activity_id = 19; // represents the activity ID for deleting a van
          ...
          $errors['error'] = "Deletion failed: " . $stmt->error;
      }
      $stmt->close();
  }
}
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <input type="text" name="plate_number" placeholder="Plate Number" class="form-control" value="<?php echo isset($van['plate_number']) ? $van['plate_number'] : '';?>" disabled>
            </div>

            <div class="form-group">
              <input type="text" name="model" placeholder="Model" class="form-control" value="<?php echo isset($van['model']) ? $van['model'] : '';?>" disabled>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-md-6">
            <div class="form-group">
              <input type="text" name="color" placeholder="Color" class="form-control" value="<?php echo isset($van['color']) ? $van['color'] : '';?>" disabled>
            </div>
